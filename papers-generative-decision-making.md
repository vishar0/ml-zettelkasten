...
- [ ] [2024] JaxGCRL: Accelerating Goal-Conditioned RL Algorithms and Research - [paper](https://arxiv.org/abs/2408.11052), [code](https://github.com/MichalBortkiewicz/JaxGCRL)
- [x] [2022] [JimmyBa] You Can't Count on Luck: Why Decision Transformers and RvS Fail in Stochastic Environments - [paper](https://arxiv.org/abs/2205.15967)
...
- [x] [2021] [GDM,PedroOrtega,Nando] Shaking the Foundations: Delusions in Sequence Models for Interaction and Control - [paper](https://arxiv.org/abs/2110.10819)
...
- **tl;dr**: The theory-of-failure paper for this entire file — using a sequence model *as a policy* (Decision Transformer, goal/return-conditioning, behavior cloning from demos) breaks whenever latent confounders exist, because the model treats its own sampled actions as evidence about the world; the fix is causal: condition on observations, **intervene** (do) on self-generated actions.
  - Under a hidden task parameter $\theta$ (a confounder of actions and observations), conditioning on your own sampled action collapses the posterior — $P(\theta \mid a)$ acts as if an expert who *saw* $\theta$ chose $a$ — so the agent becomes certain of a world state it invented (self-delusion); prediction and imitation are only equivalent when nothing is latent.
  - Treating past self-generated actions as interventions, $P(A_{t+1} \mid \mathrm{do}(a_{1:t}), o_{1:t})$, cuts the $a \to \theta$ evidence flow so the agent learns only from its actions' *effects* — and the resulting adaptive policy is Thompson sampling, derived from first principles rather than as a heuristic. Remark 6 is a direct hit on return/goal-conditioned policies: *choosing* the goal/return to condition on is itself an action, so conditioning on it is delusion.
  - Practical recipe: **factual teaching** (log-loss on observed data) trains conditioning; **counterfactual teaching** (score the model's action prediction against the expert's revealed action, stop-gradient through the sampled action) trains intervening — but this needs an expert available *online*; doing it purely from offline demonstrations is an open problem, since demos are confounded by $\theta$ (you can't continue a trajectory whose expert action you didn't take).
- **Abstract**:
...
- **Intro highlights** (verbatim):
  - The setup that breaks (build a model from demonstrations, then act by sampling your own actions from it):
    - > can we use this model for iteratively choosing the next action ... by sampling it from $a_{t+1} \sim P(A_{t+1} \mid a_{1:t}, o_{1:t})$ ... it would allow the construction of highly-general adaptive agents by simply building a probabilistic model from observed demonstrations ... without relying on expensive reward maximization
  - > the answer to this question is "no" ... **actions ... must be treated as causal interventions.** ... **the model update triggered by the collected data differs depending upon whether the data was generated by the model itself (i.e. actions) or outside of it (i.e. observations), and mixing them up leads to wrong inferences.** These take the form of **self-delusions where the model takes its own actions as evidence about the world** (for instance, believing that "Cheeseburgers are on the menu" after sampling this sentence ...) due to the presence of confounding variables.

- **Self-delusion** (the mechanism, intuitively)
  - The cheeseburger example: an assistant says "cheeseburgers are on the menu," then on the next turn reads its own sentence back as if a user had said it, and stays convinced. It confuses "I generated this" with "the world told me this."
  - Why it happens: a hidden variable (the latent task/config) is a **common cause** of both the action and the outcome. Conditioning on your own action lets information flow *backward* through that common cause and corrupt your belief about the outcome. It is a confounding / backdoor problem, not a capacity problem, so more scale or data does not fix it.

- **Prize-or-frog toy (Fig. 1) and the four cases**
  - Setup: a latent config $\Theta$ decides which of two boxes hides a prize; an expert *told* $\Theta$ opens the right box. Variables: $\Theta$ (latent), $A$ (box opened), $O$ (prize / frog). Joint $P(\Theta, A, O) = P(\Theta)\,P(A \mid \Theta)\,P(O \mid \Theta, A)$.
  - The delusion appears in exactly one of {fully, partially observable} × {passive-predict, active-act}:
    - Passive (predict the expert), either observability: fine, because conditioning on the *expert's* action is a valid update (the expert's action really does depend on $\Theta$).
    - Fully observable + active (you see $\Theta$, then act): fine, no hidden variable left to be wrong about.
    - **Partially observable + active** ($\Theta$ hidden and *you* generate the action): **delusion.** Your action is a blind guess, yet the model updates its belief about $\Theta$, and thus $O$, as if the action had revealed $\Theta$.
  - Intuitively (needs both): you need something hidden to be wrong about (partial observability) *and* a self-chosen action that does not actually know it (active). Remove either and there is nothing to fool yourself with.

- **The fix: your own actions are interventions, not observations (Fig. 2)**
  - The one bit of math worth keeping: conditioning gives $P(\Theta \mid a, o) \propto P(\Theta)\,P(a \mid \Theta)\,P(o \mid \Theta, a)$; intervening overwrites the action mechanism with your own choice, deleting the $P(a \mid \Theta)$ factor, so $P(\Theta \mid \mathrm{do}(a), o) \propto P(\Theta)\,P(o \mid \Theta, a)$. In particular $P(\Theta \mid \mathrm{do}(a)) = P(\Theta)$: choosing an action tells you nothing about the latent.
  - Intuitively (graph surgery): $\mathrm{do}(a)$ cuts the arrow going *into* $A$, so no information flows from your action back to the confounder $\Theta$; only the honest forward effect $A \to O$ survives. You still learn from the *outcome* $o$ (the world's response), just not from the act of choosing.
  - Caveat: evaluating an intervention requires knowing the causal structure (which way the arrows point); conditioning needs no such assumption. So intervening buys correctness at the cost of a causal assumption.

- **The rule (summary)**
  - > actions (i.e. data generated by the model) are causal interventions, whilst observations (i.e. data generated externally) are standard Bayesian conditions ... **one cannot learn from one's own actions, but only from their effects.**
  - Provenance (who generated a token) is what decides evidence vs intervention, even when the strings are identical.

- **Notable remarks** (one-liner + intuition each)
  - Holds for stochastic experts and outcomes too, not just the deterministic toy.
  - **Why classical RL is immune**: it never treats its own action as a random variable to condition on; an action just *indexes* a family of distributions. The policy generates actions and RL reasons about their *effects* ($\mathrm{do}(a)$); it never multiplies a policy-likelihood $P(a \mid \theta)$ into a belief update over the latent, so the delusion cannot arise.
  - **Thompson sampling falls out**: the correct way to sample an action under $\mathrm{do}$ is "sample $\Theta \sim P(\Theta)$, then act $a \sim P(a \mid \Theta)$", i.e. Thompson sampling derived from first principles rather than as a heuristic.
  - **Goal-/return-conditioned planning is a self-delusion**: conditioning on a desired goal or return $G$ is itself choosing an action, so it leaks false information about the latent task. This is the causal root of why Decision-Transformer-style return conditioning is fragile (see the You Can't Count on Luck entry above for the stochastic-environment version).
  - **Rich observations can dodge it**: if the observation stream implicitly reveals the task identifier, the problem collapses toward the fully-observable case and the delusion shrinks.
...
- **Intro**:
  ...
  - The Pearl connection → the loss mask (the load-bearing paragraph):
    - > This provenance distinction is exactly **Pearl's distinction between conditioning and intervention** [12, 13]. Observing an action-like event $a$ gives the conditional distribution $P(o \mid a)$: it asks what tends to be true in records where that event occurred. Setting the action ourselves gives $P(o \mid \mathrm{do}(a))$: it asks what follows after the action mechanism has been overwritten. For a deployed agent, its own turns belong to the second category. **The agent should condition on what it did when predicting later world responses, but it should not update as if its own attempt were independent evidence about the world.** In a causal language-model loss, the translation is simple: **keep self-authored tokens in the input prefix, but mask them out of the target labels.** World-written tokens - user messages, teacher demonstrations, tool outputs, verifier verdicts, and oracle corrections - remain supervised evidence.
...
- **1.1 Bayes under intervention (the minimal model)**
  - A latent program $p$ (task, hypothesis, schema, or "expert") drives both the chosen action and the resulting observation. The joint factors as $P(p, a, o) = P(p)\,P(a \mid p)\,P(o \mid p, a)$.
  - **Observing** an action $a = a^\star$ (someone else acted), the Bayes posterior keeps the action-likelihood factor as evidence:
    - $P(p \mid a^\star, o) \propto P(p)\,P(a^\star \mid p)\,P(o \mid p, a^\star)$
  - **Intervening** $\mathrm{do}(a^\star)$ (the agent set the action itself), the action mechanism $P(a \mid p)$ is deleted and replaced by a point mass $\delta_{a^\star}(a)$. Marginalizing the action out:
    - $P(o \mid \mathrm{do}(a^\star)) = \sum_p P(p)\,P(o \mid p, a^\star)$
    - and after the outcome arrives, the latent updates only through the outcome channel: $P(p \mid \mathrm{do}(a^\star), o) \propto P(p)\,P(o \mid p, a^\star)$.
  - The whole fix is one missing factor: the interventional posterior drops $P(a^\star \mid p)$. The paper's one-line statement:
  ...
  - Caveat the paper flags (Remark): to evaluate an intervention you must know the causal structure. If the graph were $a \to p$ (action precedes the latent) instead of $p \to a$, choosing the action *would* legitimately fix the latent. So the rule depends on the assumption that a real actor's action reflects the latent ($p \to a$), which is what makes a self-chosen action uninformative about it.
...
- **1.2 The loss mask (translating the rule to an LLM)**
  - Instantiate two fresh copies of a base LLM, $p_{\theta^{\mathrm{obs}}}$ and $p_{\theta^{\mathrm{do}}}$, fine-tuned on the **same** data with the **same** optimizer. The only difference is which tokens contribute to the loss. Let $\gamma_i \in \{0,1\}$ gate slot $i$ ($\gamma_i = 1$ for the agent, $0$ for the world).
    - **Observational**: $\mathcal{L}^{\mathrm{obs}}(\theta) = -\tfrac{1}{T}\sum_{i=1}^{T} \log p_\theta(z_i \mid z_{<i}, c)$ — supervise every token, including the agent's own turns (this is the ordinary causal-LM loss, i.e. the observational likelihood).
    - **Interventional**: $\mathcal{L}^{\mathrm{do}}(\theta) = -\tfrac{1}{|\{i : \gamma_i = 0\}|}\sum_{i : \gamma_i = 0} \log p_\theta(o_i \mid z_{<i}, c)$ — supervise only world-written tokens; the agent's own turns are dropped from the sum.
  - > Both models still see the agent's turns in the input context. They are only excluded from the gradient.
  - Pretraining and ordinary SFT are the special case where everything is world-written, so $\mathcal{L}^{\mathrm{do}}$ reduces to the standard LM loss.
- **1.3 Prediction lemma (why conditioning on your own actions still helps)**
  - Split the information sets: coarse $\mathcal{G}_0 = \sigma(\text{teacher turns})$ vs rich $\mathcal{G}_1 = \sigma(\text{teacher turns, agent actions, environment turns})$, with $\mathcal{G}_0 \subseteq \mathcal{G}_1$. Let $m_0 = \mathbb{E}[t \mid \mathcal{G}_0]$, $m_1 = \mathbb{E}[t \mid \mathcal{G}_1]$ for a future world token $t$.
  - The richer predictor is weakly better in squared error: $\mathbb{E}[(t-m_1)^2] \le \mathbb{E}[(t-m_0)^2]$, with the exact decomposition $\mathbb{E}[(t-m_0)^2] = \mathbb{E}[(t-m_1)^2] + \mathbb{E}[(m_1-m_0)^2]$. Strict whenever the extra variables (the agent's own actions and side info) change the conditional mean.
...
- **2. Experiment**
  - **Model / data**: Qwen2.5-0.5B with LoRA adapters. Synthetic single-formula STEM generator, deterministic numeric verifier. Three domains (physics, chemistry, materials), ten skills (Ohm's law voltage and current, electric power, density, specific heat, molarity, pH, kinetic energy, engineering stress, first-order Bragg diffraction). Splits $|\mathcal{D}_{\mathrm{SFT}}| = 200$, $|\mathcal{D}_{\mathrm{agent}}| = 200$, $|\mathcal{D}_{\mathrm{test}}| = 100$.
  - **Verifier** (binary): $\mathrm{Solve}(x, y) = \mathbb{1}\{\, |\hat{\alpha}(y) - \alpha| \le \max(\epsilon_{\mathrm{abs}}, \epsilon_{\mathrm{rel}} \cdot |\alpha|) \,\}$, extracting the number after a `Final answer:` marker, plus a unit check when a unit is given.
  - **Interaction stream**: $x \to \hat{y} \to v \to y^\star$ where $x$ = question, $\hat{y}$ = agent attempt, $v$ = verifier verdict, $y^\star$ = oracle correction (only when $\hat{y}$ is wrong). Causal reading: $\hat{y}$ is an action, so read it as $\mathrm{do}(\hat{y})$; $v$ and $y^\star$ are world-written observations, so they are evidence.
  - **Wrong answers are the model's own**: the frozen one-epoch SFT policy samples attempts on $\mathcal{D}_{\mathrm{agent}}$ until it accumulates 400 wrong ones (785 turns, of which 385 first attempts were correct). The dataset itself contains only correct oracle solutions; the errors are genuine model rollouts, not injected labels.
  - **Methods compared** (all start from the same SFT seed, differ only in how verifier information becomes training data):
    - `SFT`: 200 clean teacher rows, one epoch, $\mathcal{L}_{\mathrm{SFT}} = -\sum \log p(y^\star \mid x)$.
    ...
    - `SFT + GRPO`: keep the sampled group, turn binary verifier rewards $r_{jk}$ into group-relative advantages $A_{jk} = (r_{jk} - \mathrm{mean}_k)/\mathrm{std}_k$, clipped policy-gradient with a KL penalty to the SFT reference. Because the reward is binary, 24 of 50 groups are "flat" (no relative signal), leaving only 26 update steps.
    - `SFT + oracle-corrections`: on failed items, train the direct map $x \to y^\star$ ($\mathcal{L}_{\mathrm{oracle}} = -\sum_{v=0} \log p(y^\star \mid x)$). Same 400 oracle targets / 17,924 correction tokens as the interventional stream, but the wrong attempt is **not** in the context.
    - `Observational stream` ($\mathcal{L}^{\mathrm{obs}}$): supervise every token in the recorded stream, including the agent's own attempts. 51,121 supervised token positions (63.0% of 81,131).
    - `Interventional stream` ($\mathcal{L}^{\mathrm{do}}$): keep $\hat{y}$ in context but mask it; supervise only the world-written correction, i.e. the repair map $(x, \mathrm{do}(\hat{y}), v) \to y^\star$. 17,924 supervised positions (22.1%); the rest are masked context.
- **Results**
  ...
  - **Stream-matched comparison** (same frozen-seed first attempts across rows): overall solve 63 / 81 / 85 (SFT / SFT+oracle / interventional); solve-after-wrong-first-attempt 0 / 48.6 / 59.5. On the 37 held-out items the seed got wrong, the interventional repair map solves 59.5% vs 48.6% for the direct-map baseline, which the paper attributes to the prediction lemma: the repair context $(x, \mathrm{do}(\hat{y}), v)$ has the information needed to diagnose the first error, while $x \to y^\star$ does not.
...
- **Critique / caveats**
  - **Needs a correction, not just a verdict.** The repair target $y^\star$ must come from an oracle. The paper concedes: "It needs a correction or informative world response, not merely a scalar reward." Real verification is often binary or directional (pass/fail, or a preference), which supplies $v$ but not $y^\star$, so the interventional example $(x, \mathrm{do}(\hat{y}), v) \to y^\star$ cannot be formed. This also makes the GRPO comparison uneven (GRPO uses only the scalar verdict).
