...
- **Our opinion: downsides for true active / continual learning** ([[papers-continual-learning]], [[papers-open-ended-learning]]):
  ...
  - **Quality is a fixed LLM-judge prior** and the **sparsity bias trades away coverage**, both wrong defaults when future tasks are unknown.
  - **What survives.** The outer loop (cluster, cheap evaluation, predictor, iterate) is reusable if the objective becomes the learner's own loss or progress, the proxy becomes the live model, and the output becomes a policy. ClimbLab keeps cluster labels and is the substrate for that; ClimbMix is the answer to a question we are not asking.
